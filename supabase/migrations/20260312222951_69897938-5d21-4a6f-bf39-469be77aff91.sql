CREATE OR REPLACE FUNCTION auto_create_followup_on_stage_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF (TG_OP = 'UPDATE' AND NEW.funnel_stage_id IS NOT NULL 
    ...
    IF v_reason_id IS NOT NULL THEN
      ...
      -- Resolve assigned_to to user_id
      -- First try: assigned_to might already be a UUID (user_id directly)
      BEGIN
        v_user_id := NEW.assigned_to::uuid;
        -- Verify this UUID actually exists in profiles for this org
      ...
      -- Second try: match by name if UUID cast failed
      IF v_user_id IS NULL AND NEW.assigned_to IS NOT NULL AND NEW.assigned_to <> '' THEN

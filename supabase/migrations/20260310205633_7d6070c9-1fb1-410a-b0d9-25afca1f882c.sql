...
-- Add emoji usage column to ai_bots
ALTER TABLE public.ai_bots ADD COLUMN IF NOT EXISTS use_emojis boolean NOT NULL DEFAULT true;

-- Update the generate_bot_system_prompt function to include emoji preference
...
BEGIN
  ...
  v_age_text := CASE p_age_range 
    WHEN '18-25' THEN 'Use linguagem jovem, informal, moderna.' 
  ...
  IF p_company_name IS NOT NULL THEN 
    v_prompt := v_prompt || ' Você trabalha na empresa ' || p_company_name || '.'; 
  END IF;

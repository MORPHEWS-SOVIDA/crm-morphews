-- Add sales_view_all permission column to user_permissions
ALTER TABLE public.user_permissions 
ADD COLUMN IF NOT EXISTS sales_view_all boolean NOT NULL DEFAULT false;

-- Update the get_default_permissions_for_role function to include sales_view_all
...
-- Set default for existing users based on their role (admins/owners/managers get true, others false)
UPDATE public.user_permissions up
SET sales_view_all = CASE 
  WHEN om.role IN ('owner', 'admin', 'manager', 'shipping', 'finance') THEN true 
  ELSE false 
END

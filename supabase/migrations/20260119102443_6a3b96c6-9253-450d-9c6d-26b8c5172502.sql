...
BEGIN
  ...
  dashboard_perms := jsonb_build_object(
    ...
    'sales_dashboard_view', CASE WHEN p_role IN ('owner', 'admin', 'manager') THEN true ELSE false END
  );
  ...
  -- WhatsApp, AI, Demands, Instagram, Modules
  other_perms := jsonb_build_object(
    ...
    'expedition_view', CASE WHEN p_role IN ('owner', 'admin', 'expedition') THEN true ELSE false END,
    'receptive_module_access', CASE WHEN p_role IN ('owner', 'admin', 'manager', 'seller') THEN true ELSE false END,
    'integrations_view', CASE WHEN p_role IN ('owner', 'admin') THEN true ELSE false END
  );

-- Fix: accept_partner_invitation was inserting TEXT into organization_members.role (enum org_role)
-- Keep get_partner_role() as-is (returns TEXT) and cast to org_role at write time.
...
BEGIN
  ...
  v_partner_role := public.get_partner_role(v_invitation.partner_type);
  ...
  INSERT INTO public.organization_members (
    ...
    p_user_id,
    v_invitation.organization_id,
    v_partner_role::public.org_role
  )
  ON CONFLICT (user_id, organization_id) DO UPDATE
  SET role = (v_partner_role::public.org_role), updated_at = now();

  v_default_perms := public.get_default_permissions_for_role(v_partner_role);

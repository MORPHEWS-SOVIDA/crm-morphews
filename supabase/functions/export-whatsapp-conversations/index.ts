...
serve(async (req) => {
  ...
  try {
    ...
    const { dateFrom, dateTo, instanceId, conversationId } = await req.json();
    ...
    let convQuery = supabase
      ...
      .select("id, phone_number, contact_name, instance_id, display_name")
    ...
    if (!conversations?.length) {
      return new Response(
        JSON.stringify({ error: "Nenhuma conversa encontrada" }),
    ...
    // Get instance names
    const instanceIds = [...new Set(conversations.map(c => c.instance_id))];
    const { data: instances } = await supabase
      .from("whatsapp_instances")
      .select("id, name, display_name_for_team")
      .in("id", instanceIds);
    ...
    // Get profiles for sent_by_user_id mapping
    ...
    // Fetch messages in date range - batch by 1000
    ...
    let allMessages: any[] = [];
    // Process in batches of conversation IDs to avoid too large IN clauses
    const batchSize = 50;
    ...
    lines.push(`Total de conversas: ${Object.keys(msgsByConv).length}`);

...
function buildBotPersonalityPrompt(bot: AIBot): string {
  ...
  // Identidade do robô - always add name since system_prompt doesn't include it
  if (bot.name) {
    parts.push(`Você é ${bot.name}.`);
  }
  
  // Service type - add detailed instructions that system_prompt doesn't have
  if (bot.service_type) {
  ...
  // Response length - add detailed guide (system_prompt only has a basic version)
  ...
  // NOTE: Gender, age_range, brazilian_state, regional_expressions, company_differential, 
  // and personality_description are NOT duplicated here because they are already included 
  // in bot.system_prompt (generated by the database trigger generate_bot_system_prompt).
  // Adding them here would cause duplicated/conflicting instructions.

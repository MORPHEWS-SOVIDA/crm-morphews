...
serve(async (req) => {
  ...
  try {
    ...
    const { data: smsPackage, error: packageError } = await supabase
      .from("sms_packages")
      .select("id, name, sms_count, price_cents, is_active")
      .eq("id", packageId)
      .eq("is_active", true)
      .single();

    if (packageError || !smsPackage) {
      ...
      line_items: [
        {
          price_data: {
            ...
            product_data: {
              ...
              description: `${smsPackage.sms_count.toLocaleString("pt-BR")} créditos SMS`,
            },
            unit_amount: smsPackage.price_cents, // Server-side price
          },
          quantity: 1,
        },
      ],
      mode: "payment",
      success_url: successUrl || `${req.headers.get("origin")}/settings/sms?success=true`,
      cancel_url: cancelUrl || `${req.headers.get("origin")}/settings/sms?canceled=true`,
      metadata: {
        type: "sms_credits",
        package_id: smsPackage.id,
        sms_count: smsPackage.sms_count.toString(),
        price_cents: smsPackage.price_cents.toString(),
    ...
    console.log("SMS checkout session created:", {
      sessionId: session.id,
      packageId: smsPackage.id,
      smsCount: smsPackage.sms_count,
      priceCents: smsPackage.price_cents,
      organizationId,
    });

...
export function useConversationDistribution() {
  ...
  const { data: orgSettings } = useQuery({
    ...
    queryFn: async () => {
      ...
      const { data } = await supabase
        .from("organizations")
        .select(
          "satisfaction_survey_enabled, satisfaction_survey_on_manual_close, satisfaction_survey_message",
        )
    ...
    mutationFn: async ({
      ...
      try {
        ...
        const { data: conv, error: convError } = await supabase
          ...
          .select(
            "lead_id, contact_name, phone_number, organization_id, instance_id",
          )
        ...
        if (convError) {
          console.warn(
            "[claimConversation] Failed to fetch conversation:",
            convError,
          );
        }
        ...
        if (conv?.organization_id) {
          ...
          if (orgError) {
            console.warn(
              "[claimConversation] Failed to fetch organization config:",
              orgError,
            );
          }
        ...
        if (
          ...
          console.log(
            "[claimConversation] Auto-linking lead by phone:",
            conv.phone_number,
          );
          ...
          const { data: existingLeads, error: existingLeadError } =
            await supabase
              ...
              .or(
                phoneCandidates
                  .map((candidate) => `whatsapp.ilike.%${candidate}%`)
                  .join(","),
              )
              .limit(10);
          ...
          if (existingLeadError) {
            console.warn(
              "[claimConversation] Failed to search existing leads:",
              existingLeadError,
            );
          }
          ...
          if (matchedLead) {
            ...
            console.log(
              "[claimConversation] Found existing lead:",
              linkedLeadId,
              matchedLead.name,
            );
          } else {
            ...
            const leadName =
              conv.contact_name || `WhatsApp ${digits.slice(-4)}`;
            const whatsappFormatted =
              phoneCandidates.find((candidate) => candidate.startsWith("55")) ||
            ...
            if (createError) {
              console.warn(
                "[claimConversation] Failed to create lead:",
                createError,
              );
            } else if (newLead) {
              linkedLeadId = newLead.id;
              console.log(
                "[claimConversation] Created new lead:",
                linkedLeadId,
                leadName,
              );
          ...
          if (linkedLeadId) {
            ...
            if (linkConversationError) {
              console.warn(
                "[claimConversation] Failed to update conversation lead_id:",
                linkConversationError,
              );
            }
            ...
            console.log(
              "[claimConversation] Lead linked to conversation:",
              linkedLeadId,
            );
          }
        }
          ...
          supabase.functions
            ...
            .then((briefingResult) => {
              if (briefingResult.data?.briefing) {
                console.log(
                  "[claimConversation] Briefing generated:",
                  briefingResult.data.briefing.substring(0, 100),
                );
            ...
            .catch((briefingError) => {
              console.warn(
                "[claimConversation] Briefing generation failed:",
                briefingError,
              );
            });
        }
        ...
        console.warn(
          "[claimConversation] Error in auto-link/briefing:",
          autoLinkError,
        );
  ...
  const closeConversation = useMutation({
    mutationFn: async (conversationId: string) => {
      ...
      if (convError || !conv) {
        console.error(
          "[closeConversation] Error fetching conversation:",
          convError,
        );
      ...
      if (orgError) {
        console.error(
          "[closeConversation] Error fetching org config:",
          orgError,
        );
      }

      const shouldSendSurvey =
        orgData?.satisfaction_survey_enabled &&
    ...
    mutationFn: async ({
      ...
      if (conv.lead_id) {
        ...
        if (!lead?.funnel_stage_id) {
          ...
          const defaultStage =
            orgConfig?.default_stage_whatsapp ||
            orgConfig?.default_stage_fallback;

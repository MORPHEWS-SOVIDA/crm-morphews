      ...
      const { data: result, error } = await supabase.functions.invoke("generate-bot-prompt", {
        body: {
          ...
          description: [
            ...
            data.products && `Produtos/Serviços: ${data.products}`,
            data.targetAudience && `Público-alvo: ${data.targetAudience}`,
            data.mainObjection && `Principal objeção dos clientes: ${data.mainObjection}`,
            data.qualificationStrategy && `Estratégia de qualificação: ${data.qualificationStrategy}`,
            data.neverDo && `O agente NUNCA deve: ${data.neverDo}`,
            data.transferReasons.length > 0 && `Transferir para humano quando: ${data.transferReasons.join(", ")}`,
            `Gênero do agente: ${data.gender}`,
            data.presentName ? `O agente se apresenta pelo nome "${data.name}"` : "O agente NÃO se apresenta pelo nome",
          ].filter(Boolean).join("\n"),
